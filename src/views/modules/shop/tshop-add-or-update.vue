    ...
    methods: {
        ...
        this.$nextTick(() => {
          ...
          if (this.dataForm.shopId) {
            this.$http({
              ...
              if (data && data.code === 0) {
                this.dataForm.createDate = data.tshop.createDate
                this.dataForm.updateDate = data.tshop.updateDate
      ...
      dataFormSubmit () {
        this.$refs['dataForm'].validate((valid) => {
          if (valid) {
            this.$http({
              url: this.$http.adornUrl(`/generator/tshop/${!this.dataForm.shopId ? 'save' : 'update'}`),
              method: 'post',
